...
do
        ...
        then
           echo "Backup table $t "
           echo "-------"
           #$MYSQLDMP -K -e  -q --single-transaction --no-create-db -h ${HOST} -u ${MUSER} -p${MPASS} ${MDB} | lbzip2 -n4 > ${PATH_DMP}/${MDB}/export_${t}_${DATEDUJ}.dmp.sql.bz2
           echo "-------"
        ...
        for p in $PART
         do
            echo "Truncating $t table from $MDB Database..."
            echo "-------"
            echo "suppression de la partition ${p} dans la table ${t}"
           ...
           echo -e " Le répertoire ${PATH_DMP} existe pas \n"
           exit 1
        fi
done
